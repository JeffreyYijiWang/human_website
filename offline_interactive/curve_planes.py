...
vec4 sample_volume(vec3 p) {
    ...
    vec4 c0 = texture(tex_array, vec3(uv, float(z0)));
    vec4 c1 = texture(tex_array, vec3(uv, float(z1)));
    vec4 c  = mix(c0, c1, t);

    if (u_bgr_input != 0) c = c.bgra; // swap rgb<->bgr
    return vec4(c.rgb, 1.0);
}

void main() {
    fragColor = sample_volume(v_tex3d);
}
"""
...
class MPRPlaneUI(mglw.WindowConfig):
    ...
    def __init__(self, **kwargs):
        ...
        self.curve_prog = self.ctx.program(vertex_shader=CURVE_VERT, fragment_shader=CURVE_FRAG)
        self.curve_prog["tex_array"].value = 0
        self.curve_prog["u_num_layers"].value = int(self.Z)

        # share same flip/bgr toggles you already use:
        self.curve_prog["u_flip_y"].value = int(getattr(self, "flip_y", 0))
        self.curve_prog["u_bgr_input"].value = int(getattr(self, "bgr_input", 1))

        # curve tool state
        self.curve_mode = False          # toggle with C
        self.curve_drawing = False
        self.curve_points_uv = []        # list of 2D points in plane UV space: (su, sv) in [-1,1] plane coords
        self.curve_points_3d = []        # list of 3D points in volume coords [0,1]^3

        # ribbon geometry (pos + tex3d)
        self.curve_vbo = self.ctx.buffer(reserve=1024 * 6 * 4)  # grows as needed
        self.curve_vao = self.ctx.vertex_array(
            self.curve_prog,
            [(self.curve_vbo, "3f 3f", "in_pos", "in_texcoord3d")]
        )
        self.curve_vertex_count = 0

        # appearance
        self.curve_width = 0.006  # in normalized volume units (tune)
        self.curve_min_dist_px = 2.0  # decimate input points in screen pixels (tune)
    ...
    def _screen_to_uv01(self, x_px, y_px):
        # ModernGL-window gives y from top; your shader expects bottom-left UV.
        W, H = self.wnd.width, self.wnd.height
        u = float(x_px) / max(W, 1)
        v = 1.0 - (float(y_px) / max(H, 1))
        return np.array([u, v], dtype=np.float32)

    def _uv01_to_plane_coords(self, uv01):
        # convert [0,1] -> [-1,1] plane coords (su, sv)
        su = (uv01[0] * 2.0) - 1.0
        sv = (uv01[1] * 2.0) - 1.0
        return np.array([su, sv], dtype=np.float32)

    def _plane_coords_to_volume_xyz(self, su_sv):
        # Map plane coords to volume position:
        # p = center + (su * scale)*u_axis + (sv * scale)*v_axis
        su, sv = float(su_sv[0]), float(su_sv[1])
        p = self.center + (su * self.scale) * self.u + (sv * self.scale) * self.v
        return np.clip(p, 0.0, 1.0)

    def _append_curve_point_from_mouse(self, x_px, y_px):
        uv01 = self._screen_to_uv01(x_px, y_px)
        su_sv = self._uv01_to_plane_coords(uv01)
        p3 = self._plane_coords_to_volume_xyz(su_sv)

        # decimate based on screen movement
        if self.curve_points_uv:
            # approximate pixel distance using uv delta * screen size
            last_uv01 = (np.array(self.curve_points_uv[-1], dtype=np.float32) * 0.5) + 0.5  # [-1,1]->[0,1]
            d = uv01 - last_uv01
            W, H = self.wnd.width, self.wnd.height
            dist_px = np.sqrt((d[0] * W)**2 + (d[1] * H)**2)
            if dist_px < self.curve_min_dist_px:
                return

        self.curve_points_uv.append((float(su_sv[0]), float(su_sv[1])))
        self.curve_points_3d.append((float(p3[0]), float(p3[1]), float(p3[2])))

    def _rebuild_curve_ribbon(self):
        # Build a camera-facing ribbon in *plane space*.
        # We keep it simple: offset each point left/right by a normal within the plane.
        pts = np.array(self.curve_points_3d, dtype=np.float32)
        if pts.shape[0] < 2:
            self.curve_vertex_count = 0
            return
        ...
        t = np.where(tn > 1e-8, t / tn, t)

        # “side” vector = normalize(cross(n, tangent))
        side = np.cross(n[None, :], t)
        sn = np.linalg.norm(side, axis=1, keepdims=True)
        side = np.where(sn > 1e-8, side / sn, side)

        w = float(self.curve_width)

        left = np.clip(pts - side * w, 0.0, 1.0)
        right = np.clip(pts + side * w, 0.0, 1.0)

        # For drawing in mesh/gizmo space, we need positions in the *same space as your volume box*.
        # In your mesh mode, you map volume [0,1] to a centered box; you already have volume_scale / transforms.
        # We'll match your gizmo convention: volume coords -> centered [-0.5,0.5].
        def vol_to_draw(p):
            return (p - 0.5)  # simple centered unit cube; if you scale volume elsewhere, adjust here.

        # Tri strip as triangles:
        verts = []
        for i in range(pts.shape[0] - 1):
            p0L, p0R = left[i], right[i]
            p1L, p1R = left[i+1], right[i+1]

            # two triangles: (p0L, p0R, p1R) and (p0L, p1R, p1L)
            tri = [
                (vol_to_draw(p0L), p0L),
                (vol_to_draw(p0R), p0R),
                (vol_to_draw(p1R), p1R),
            ...
            for pos_draw, tex3d in tri:
                verts.append([pos_draw[0], pos_draw[1], pos_draw[2], tex3d[0], tex3d[1], tex3d[2]])

        inter = np.array(verts, dtype=np.float32)
        if inter.nbytes > self.curve_vbo.size:
            self.curve_vbo.release()
            self.curve_vbo = self.ctx.buffer(reserve=int(inter.nbytes * 1.5))
            self.curve_vao = self.ctx.vertex_array(
                self.curve_prog,
                [(self.curve_vbo, "3f 3f", "in_pos", "in_texcoord3d")]
            )
        self.curve_vbo.write(inter.tobytes())
        self.curve_vertex_count = inter.shape[0]

    def _export_curve_json(self):
        if not self.curve_points_3d:
            print("[curve] nothing to export")
            return
        payload = {
            "timestamp": datetime.now().isoformat(),
            "omega_like_plane_scale": float(self.scale),
            "plane_center_01": [float(x) for x in self.center],
            "plane_u": [float(x) for x in self.u],
            "plane_v": [float(x) for x in self.v],
            "plane_n": [float(x) for x in self.n],
            "curve_points_plane_susv": self.curve_points_uv,
            "curve_points_volume_01": self.curve_points_3d,
        }
        out = Path("curve_export.json")
        out.write_text(json.dumps(payload, indent=2))
        print(f"[curve] wrote {out.resolve()}")
    ...
    def _build_hud_texture(self):
        ...
        draw.text((18, 56), "slice mode: plane + heap brush    |    mesh mode: full 3D scene, no plane used for mesh rendering", fill=(210, 220, 235, 255), font=font_small)
        ...
        draw.text((18, 124), "mesh mode draws the object inside the rectangular volume box and orbits around the object center", fill=(190, 205, 225, 255), font=font_small)
    ...
    def _load_mesh_from_path(self, path: str):
        ...
        self.mesh_view_radius = max(0.8, mesh_radius * 2.6)
    ...
    def on_key_event(self, key, action, modifiers):
        ...
        if key == self.wnd.keys.C:
            ...
            print(f"[curve] mode={'ON' if self.curve_mode else 'OFF'}")
            return

        if key == self.wnd.keys.P:
            self._export_curve_json()
            return

        if key == self.wnd.keys.BACKSPACE:
            self.curve_points_uv.clear()
    ...
    def _render_mesh_view(self):
        ...
        if self.curve_vertex_count > 0:
            self.curve_prog["u_mvp"].write((P @ V).astype(np.float32).tobytes())
            self.curve_prog["u_num_layers"].value = int(self.Z)
            self.curve_prog["u_flip_y"].value = int(self.flip_y)
            self.curve_prog["u_bgr_input"].value = int(self.bgr_input)
            self.tex.use(location=0)
            self.curve_vao.render(mode=moderngl.TRIANGLES, vertices=self.curve_vertex_count)
    ...
    def _render_gizmo(self):
        ...
        if self.curve_vertex_count > 0:
            self.curve_prog["u_mvp"].write(MVP.tobytes())  # gizmo MVP already computed there
            self.curve_prog["u_flip_y"].value = int(self.flip_y)
            self.curve_prog["u_bgr_input"].value = int(self.bgr_input)
            self.tex.use(location=0)
            self.curve_vao.render(mode=moderngl.TRIANGLES, vertices=self.curve_vertex_count)
