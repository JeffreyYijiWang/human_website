...
uniform mat4 u_mesh_to_volume;

in vec3 in_pos;       // draw-space position used by the orbital camera
in vec3 in_obj_pos;   // original mesh-space position used for volume lookup
in vec3 in_normal;
...
void main() {
    v_voluvw = (u_mesh_to_volume * vec4(in_obj_pos, 1.0)).xyz;
...
def affine_bbox_to_unit(bmin: np.ndarray, bmax: np.ndarray) -> np.ndarray:
    """Map original mesh-space coordinates into [0,1]^3 volume coordinates."""
    extent = np.maximum(bmax - bmin, 1e-6).astype(np.float32)
    M = np.eye(4, dtype=np.float32)
    M[0, 0] = 1.0 / extent[0]
    M[1, 1] = 1.0 / extent[1]
    M[2, 2] = 1.0 / extent[2]
    M[0, 3] = -bmin[0] / extent[0]
    M[1, 3] = -bmin[1] / extent[1]
    M[2, 3] = -bmin[2] / extent[2]
    return M
...
class MPRPlaneUI(mglw.WindowConfig):
    gl_version = (3, 3)
    title = "MPR Plane UI + Mesh Volume Mapping (U)"
    ...
    def __init__(self, **kwargs):
        ...
        self.hud_vao = self.ctx.vertex_array(self.hud_prog, [(self.hud_vbo, "2f 2f", "in_pos", "in_uv")])
        ...
        self.mesh_to_volume = np.eye(4, dtype=np.float32)
    ...
    def _build_hud_texture(self):
        img = Image.new("RGBA", (980, 180), (0, 0, 0, 0))
        ...
        draw.rounded_rectangle((0, 0, 979, 179), radius=18, fill=(15, 18, 25, 180))
        draw.text((18, 16), "U: upload / toggle mesh   |   Shift+U: choose another mesh", fill=(255, 255, 255, 255), font=font)
        draw.text((18, 56), "mesh mode: real 3D orbital camera + volume mapped onto the mesh surface", fill=(210, 220, 235, 255), font=font_small)
        draw.text((18, 90), "mapping uses original mesh-space positions -> mesh_to_volume transform -> 3D volume sample", fill=(190, 205, 225, 255), font=font_small)
        draw.text((18, 124), "the plane is only used in slice mode; mesh mode samples the volume directly on the uploaded mesh", fill=(190, 205, 225, 255), font=font_small)
    ...
    def _load_mesh_from_path(self, path: str):
        ...
        # draw-space geometry for the orbital camera
        verts_draw = (verts - center) / max_extent

        # original mesh-space positions for volume mapping
        verts_obj = verts.copy()
        self.mesh_to_volume = affine_bbox_to_unit(bmin, bmax)

        tri_pos = verts_draw[faces].reshape(-1, 3)
        tri_obj = verts_obj[faces].reshape(-1, 3)
        tri_nrm = norms[faces].reshape(-1, 3)

        interleaved = np.hstack([tri_pos, tri_obj, tri_nrm]).astype("f4", copy=False)
        ...
        self.mesh_vao = self.ctx.vertex_array(
            self.mesh_prog,
            [(self.mesh_vbo, "3f 3f 3f", "in_pos", "in_obj_pos", "in_normal")],
        )
    ...
    def _render_mesh_view(self):
        ...
        self.mesh_prog["u_mesh_to_volume"].write(self.mesh_to_volume.astype(np.float32).tobytes())
